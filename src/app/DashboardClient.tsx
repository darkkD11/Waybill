...
import { Users, Receipt, TableProperties, FileSpreadsheet, Truck, LogOut, ShieldCheck } from 'lucide-react';
import { logout } from './login/actions';

type TabType = 'bilty' | 'clients' | 'history' | 'register' | 'export';

export default function DashboardClient({ isAdmin, userEmail }: { isAdmin: boolean; userEmail: string }) {
        ...
        <aside className="w-full md:w-64 bg-slate-900 text-white flex flex-col border-r border-slate-800 shrink-0 shadow-lg">
          ...
          <nav className="flex-1 p-4 space-y-1">
            ...
            {isAdmin && (
              <a
                href="/admin"
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition duration-200 cursor-pointer text-amber-400 hover:bg-slate-800"
              >
            ...
            <form action={logout}>
              <button
                type="submit"
                className="w-full flex items-center gap-3 px-4 py-3 mt-4 rounded-xl text-sm font-semibold transition duration-200 cursor-pointer text-red-400 hover:bg-red-900/30"
              >
          ...
          <div className="p-4 border-t border-slate-800 text-[10px] text-slate-500 text-center font-medium">
            Signed in as: <br />
            <span className="text-slate-300">{userEmail}</span><br /><br />
